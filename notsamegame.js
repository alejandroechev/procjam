...
function readjustGrid() {
  ...
  for (var j = 0; j < numColumns; j++) {
    ...
    for (var i = 0; i < newColumnColors.length; i++) {
      ...
      grid[numRows - i - 1][j] = new Tile(xy.x, xy.y, numRows - i - 1, j,
        colors[newColumnColors[i]], newColumnColors[i]);
    }
  ...
  for (var i = 0; i < numRows; i++) {
    ...
    for (var j = 0; j < newRowColors.length; j++) {
      ...
      grid[i][j] = new Tile(xy.x, xy.y, i, j,
        colors[newRowColors[j]], newRowColors[j]);
    }
